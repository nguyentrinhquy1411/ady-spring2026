...
\email{\{nguyentrinhquy1411, nguyenminhfptu, nguyendonhatquangaidev, lephamhuechi\}@gmail.com}}
...
\begin{figure}[!ht]
\centering
\begin{tikzpicture}[node distance=0.8cm and 1.2cm, font=\scriptsize]
    % Row 1
    \node (B) [data] {Log Returns};
    \node (A) [data, left=of B] {Raw Dataset};
    \node (C) [data, right=of B] {Eng. Features};

    % Row 2
    \node (E) [preprocess, below=1.2cm of B] {Temporal Split};
    \node (D) [preprocess, left=of E] {Scaling};
    \node (F) [preprocess, right=of E] {ML Ensemble};

    % Row 3 (Parallel)
    \node (F3) [model, below=1.2cm of E] {Boosting (XG/L)};
    \node (F1) [model, left=of F3] {Linear \& RF};
    \node (F5) [model, right=of F3] {LSTM Network};

    % Row 4
    \node (G) [eval, below=1.2cm of F3] {Evaluation};
    \node (H) [eval, right=of G] {Metrics (RMSE/$R^2$)};

    % Phase arrows
    \draw [arrow] (A) -- (B);
    \draw [arrow] (B) -- (C);
    
    % Main centralized transition
    \draw [arrow] (B.south) -- (E.north);
    
    % Lateral branches Row 1 -> Row 2
    \draw [arrow] (A.south) -- ++(0,-0.3) -| (D.north);
    \draw [arrow] (C.south) -- ++(0,-0.3) -| (F.north);

    % Row 2 actions
    \draw [arrow] (D) -- (E);
    \draw [arrow] (E) -- (F);
    
    % Centered Fork Row 2 to Row 3
    \draw [arrow] (E.south) -- (F3.north);
    \draw [arrow] (E.south) -- ++(0,-0.4) -| (F1.north);
    \draw [arrow] (E.south) -- ++(0,-0.4) -| (F5.north);
    
    % Centered Fork Row 3 to Row 4
    \draw [arrow] (F3.south) -- (G.north);
    \draw [arrow] (F1.south) -- ++(0,-0.4) -| (G.north);
    \draw [arrow] (F5.south) -- ++(0,-0.4) -| (G.north);
    \draw [arrow] (G) -- (H);
\end{tikzpicture}
\caption{Premium research methodology workflow for housing price prediction.}
\label{fig:pipeline}
\end{figure}
