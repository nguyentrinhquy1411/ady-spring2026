...
\begin{table}[!ht]
\caption{Descriptive statistics of the ZHVI dataset across full, training, and test periods.}
...
\begin{table}[t]
\caption{Model hyperparameters and approximate single-machine training times.}
...
(RMSE $0.00515$, R$^2$ $0.569$). Figure~\ref{fig:model_comparison}
visualises the cross-model ranking on all three metrics.
...
\$1{,}630.50 (LSTM). Figure~\ref{fig:pred_vs_actual} illustrates
predicted versus actual prices for representative metro areas.
For comparison, the FST-SM of
Chen and Zheng~\cite{ref_fmem2023} reports RMSE $1.0580$
($\times 10{,}000$ USD $= \$10{,}580$) and MAE $0.9051$
($\times 10{,}000$ USD $= \$9{,}051$), measured on price levels
for 101 San Francisco communities and thus not directly comparable
to our national log-return benchmark.


\setlength{\tabcolsep}{4pt}
\begin{table}[t]
\caption{One-step-ahead forecast performance comparison across all models. Bold denotes the best result per metric. FST-SM results are price-level only and not directly comparable (see footnote).}
\label{tab:results}
\centering
\resizebox{\linewidth}{!}{%
...
\multicolumn{8}{@{}l}{\footnotesize
  $^{\star}$~Chen \& Zheng~\cite{ref_fmem2023}: price-level MAE\,$0.9051$,
  RMSE\,$1.0580$ ($\times10{,}000$\,USD), 101~SF communities; not}\\
\multicolumn{8}{@{}l}{\footnotesize
  directly comparable to our national log-return evaluation.}
\end{tabular}}
\end{table}
...
The FST-SM excels at three tasks: (i)~\emph{spatial borrowing of strength}, where the CAR prior lets data-sparse communities leverage information from geographic neighbours; (ii)~\emph{uncertainty quantification}, as MCMC posterior distributions yield credible intervals on predictions and fixed effects; and (iii)~\emph{interpretability}, since eigenfunction decompositions reveal the dominant spatiotemporal patterns. ML models are complementary along different axes: gradient-boosted pipelines (LightGBM, XGBoost) train on hundreds of thousands of observations in seconds and ingest additional tabular inputs---mortgage rates, employment figures---without architectural changes, while accessible frameworks (scikit-learn, PyTorch) lower the barrier to production deployment.
...
Following the reviewer recommendation and our single-step baseline, we extend the evaluation to 2-, 6-, and 12-month direct-forecast horizons. For each horizon $h \in \{2, 6, 12\}$, the target is $r_{s,t+h} = \log(p_{s,t+h}/p_{s,t+h-1})$ and the same six lag features (computed at time $t$) serve as inputs. Models are retrained from scratch under the identical temporal split; no future information leakage occurs. Table~\ref{tab:multistep} reports log-return RMSE and R$^2$ averaged across all 894 metro areas.

\begin{table}[!ht]
\caption{Multi-step-ahead log-return RMSE and R$^2$ for $h \in \{2, 6, 12\}$ months. Bold denotes the best result per column.}
\label{tab:multistep}
\centering
\resizebox{\linewidth}{!}{%
...
\toprule
\textbf{Model}
  & \multicolumn{2}{c}{\textbf{$h=2$}}
  & \multicolumn{2}{c}{\textbf{$h=6$}}
  & \multicolumn{2}{c}{\textbf{$h=12$}} \\
\cmidrule(lr){2-3}\cmidrule(lr){4-5}\cmidrule(lr){6-7}
 & RMSE & R$^2$ & RMSE & R$^2$ & RMSE & R$^2$ \\
\midrule
Linear Reg.   & \textbf{.00431} & \textbf{.614} & .00892 & .358 & .01104 & .147 \\
Random Forest & .00449 & .583  & .00871 & .388 & .01083 & .176 \\
LightGBM      & .00441 & .597  & \textbf{.00858} & \textbf{.406} & \textbf{.01061} & \textbf{.208} \\
XGBoost       & .00453 & .577  & .00864 & .398 & .01069 & .197 \\
LSTM          & .00618 & .382  & .01053 & .211 & .01287 & .021 \\
\bottomrule
\end{tabular}}
\end{table}

Several patterns emerge. First, forecast accuracy degrades sharply as the horizon lengthens: R$^2$ drops from approximately $0.60$ at $h=2$ to below $0.21$ at $h=12$ for all models, confirming that ZHVI log-returns are largely unpredictable beyond a few months. Second, the ranking flips at longer horizons: while Linear Regression dominates at $h=2$, LightGBM becomes the best model at $h=6$ and $h=12$, indicating that non-linear lag interactions gain marginal relevance as the autocorrelation signal from \texttt{lag\_1} decays. Third, LSTM continues to lag, with near-zero R$^2$ at $h=12$, consistent with its difficulty in handling the sparse long-range signal in a global pooling regime. These results suggest that practitioners requiring medium-to-long-horizon forecasts should prefer gradient-boosted ensembles over both linear and recurrent architectures.

\subsection{Limitations}
\label{sec:limitations}

Our study has three remaining limitations that guide future work. Feature scope is restricted to autoregressive log-return lags; incorporating macroeconomic covariates (unemployment, 30-year mortgage rates, housing permits) should improve all models. The global LSTM represents a conservative deep-learning baseline; region-specific LSTMs, temporal transformers, or spatial graph networks may recover additional predictive signal. Finally, the direct multi-step evaluation above assumes fixed lag features computed at time $t$; iterative and sequence-to-sequence strategies could provide complementary insights.
